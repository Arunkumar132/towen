...
const InfiniteRatingScroll: React.FC<InfiniteRatingScrollProps> = ({ className }) => {
	...
	const marqueeItems = React.useMemo(() => (baseItems.length > 0 ? [...baseItems, ...baseItems] : []), [baseItems]);
	...
	const scrollDuration = React.useMemo(() => Math.max(22, baseItems.length * 4), [baseItems.length]);
	...
	return (
		<section className={`bg-slate-950 text-white py-20 relative overflow-hidden ${className ?? ''}`}>
			<div className="absolute inset-0 bg-[radial-gradient(circle_at_top,_rgba(255,255,255,0.08),_transparent)]" aria-hidden="true" />
			<div className="relative w-full">
				<div className="px-4 sm:px-8 lg:px-16 xl:px-20 2xl:px-28">
					<div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
						<div>
							<p className="text-sm uppercase tracking-[0.45em] text-purple-200">Feedbacks</p>
							<h2 className="text-3xl sm:text-4xl font-bold mt-3">Loved by foodies across the city</h2>
							<p className="mt-3 text-slate-300 max-w-2xl">
								Hear straight from subscribers, delivery partners, and chefs who trust us with their everyday meals.
							</p>
						</div>
						<div className="flex items-center gap-4">
							<div className="rounded-2xl bg-white/10 px-6 py-4 backdrop-blur border border-white/10">
								...
								<p className="text-xs text-slate-300 mt-1">
									{baseItems.length > 0 ? `${approved.length}+ verified reviews` : 'Waiting for first public review'}
								</p>
							</div>
						</div>
					</div>
									...
									>
										<div className="flex items-center justify-between gap-4">
											<div>
												...
												<p className="text-sm text-purple-200">{item.role}</p>
											</div>
										...
										<p className="mt-4 text-sm text-slate-200 leading-relaxed">“{item.feedback}”</p>
										{item.createdAt ? (
											<p className="mt-4 text-xs text-slate-400">{formatDate(item.createdAt)}</p>
										) : null}
									</article>
								))}
							</div>
